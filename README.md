...
-Body Management (O(1)): Utilizes std::deque<pair<int, int>> for constant-time operations when adding a head and removing a tail, ensuring consistent performance regardless of snake length.
-Collision Detection (O(N)): Self-collision logic scales linearly with the snake's length (N), maintaining high frame rates on minimal hardware.
